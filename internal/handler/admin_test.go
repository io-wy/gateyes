...
func assertSuccess(t *testing.T, rec *httptest.ResponseRecorder) map[string]any {
	t.Helper()
	var payload map[string]any
	if err := json.Unmarshal(rec.Body.Bytes(), &payload); err != nil {
	...
	if payload["success"] != true {
		t.Fatalf("expected success=true, got payload: %s", rec.Body.String())
	}
	if code, ok := payload["code"].(float64); !ok || code != 20000 {
		t.Fatalf("expected code=20000, got payload: %s", rec.Body.String())
	}
	return payload
}
...
func TestAdminUserLifecycleAndDashboardEndpoints(t *testing.T) {
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("POST /admin/providers status = %d, want %d: %s", rec.Code, http.StatusOK, rec.Body.String())
	}
	runtimeProvider := assertSuccess(t, rec)
	...
	updatedProvider := assertSuccess(t, rec)
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("POST /admin/projects status = %d, want %d: %s", rec.Code, http.StatusOK, rec.Body.String())
	}
	projectPayload := assertSuccess(t, rec)
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("POST /admin/projects(policy) status = %d, want %d: %s", rec.Code, http.StatusOK, rec.Body.String())
	}
	projectPolicyPayload := assertSuccess(t, rec)
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("POST /admin/users status = %d, want %d: %s", rec.Code, http.StatusOK, rec.Body.String())
	}
	createPayload := assertSuccess(t, rec)
	data := createPayload
...
func TestSuperAdminTenantRoutesAndPublicEndpoints(t *testing.T) {
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("POST /admin/tenants status = %d, want %d: %s", rec.Code, http.StatusOK, rec.Body.String())
	}
	tenantPayload := assertSuccess(t, rec)
...
func TestAdminAPIKeysUsageAndRouteTraceEndpoints(t *testing.T) {
	...
	userPayload := assertSuccess(t, rec)
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("POST /admin/keys status = %d, want %d: %s", rec.Code, http.StatusOK, rec.Body.String())
	}
	keyPayload := assertSuccess(t, rec)
	...
	updatedKey := assertSuccess(t, rec)
	...
	rotatedKey := assertSuccess(t, rec)
	...
	tracePayload := assertSuccess(t, rec)
	...
	responseList := assertSuccess(t, rec)["items"].([]any)
	...
	summaryPayload := assertSuccess(t, rec)
	...
	breakdownPayload := assertSuccess(t, rec)
	...
	revokedKey := assertSuccess(t, rec)
	...
	auditPayload := assertSuccess(t, rec)["items"].([]any)
...
func TestAdminServiceLifecycleAndSubscriptionReviewEndpoints(t *testing.T) {
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("POST /admin/services status = %d, want %d: %s", rec.Code, http.StatusOK, rec.Body.String())
	}
	servicePayload := assertSuccess(t, rec)
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("POST /admin/services/:id/versions status = %d, want %d: %s", rec.Code, http.StatusOK, rec.Body.String())
	}
	secondVersionID := assertSuccess(t, rec)["id"].(string)
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("POST /admin/services/:id/subscriptions status = %d, want %d: %s", rec.Code, http.StatusOK, rec.Body.String())
	}
	subscriptionID := assertSuccess(t, rec)["id"].(string)
	...
	reviewPayload := assertSuccess(t, rec)
...
func TestAdminVirtualKeyLifecycle(t *testing.T) {
	...
	apiKeyData := assertSuccess(t, apiKeyList)["data"].([]any)
	if len(apiKeyData) == 0 {
	...
	if createResp.Code != http.StatusCreated {
		t.Fatalf("POST /admin/virtual-keys status = %d, want %d: %s", createResp.Code, http.StatusOK, createResp.Body.String())
	}
	createPayload := assertSuccess(t, createResp)
	...
	listPayload := assertSuccess(t, listResp)["items"].([]any)
	...
	getPayload := assertSuccess(t, getResp)
	...
	updatePayload := assertSuccess(t, updateResp)
...
func TestAdminDeleteTenantCascade(t *testing.T) {
	...
	if got, err := env.store.GetTenant(ctx, "tenant-del"); err != nil {
	...
	for _, item := range assertSuccess(t, rec)["data"].([]any) {
		m := item.(map[string]any)
		if m["id"] == "tenant-del" && m["status"] != repository.StatusInactive {
			t.Fatal("deleted tenant should be inactive")
		}
	}
...
func TestAdminDeleteProjectCascade(t *testing.T) {
	...
	projectID := assertSuccess(t, rec)["id"].(string)
	...
	if got, err := env.store.GetProject(ctx, "tenant-a", projectID); err != nil {

...
func TestCreateServiceVersion(t *testing.T) {
	...
	if rec.Code != http.StatusCreated {
		t.Fatalf("POST version status = %d, want %d: %s", rec.Code, http.StatusCreated, rec.Body.String())
	}
	data := decodeBodyMap(t, rec)["data"].(map[string]any)
	if data["id"] == "" {
...
func TestRollbackServiceVersion(t *testing.T) {
	...
	serviceID := decodeBodyMap(t, rec)["data"].(map[string]any)["service"].(map[string]any)["id"].(string)
	versionID := decodeBodyMap(t, rec)["data"].(map[string]any)["initial_version"].(map[string]any)["id"].(string)

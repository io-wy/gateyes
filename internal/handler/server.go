...
func NewServer(cfg config.ServerConfig, h *Handler, adminH *AdminHandler, mw *middleware.Middleware, ingressMW *ingress.Middleware, tlsManager *ingress.TLSManager) *Server {
	...
	admin.Use(mw.RequireRoles(repository.RoleTenantAdmin, repository.RoleSuperAdmin))
	{
		...
		admin.GET("/virtual-keys", adminH.ListVirtualKeys)
		admin.POST("/virtual-keys", adminH.CreateVirtualKey)
		admin.GET("/virtual-keys/:id", adminH.GetVirtualKey)
		admin.PUT("/virtual-keys/:id", adminH.UpdateVirtualKey)
		admin.DELETE("/virtual-keys/:id", adminH.DeleteVirtualKey)

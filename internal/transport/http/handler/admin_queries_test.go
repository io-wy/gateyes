...
func TestDeleteVirtualKey(t *testing.T) {
	...
	vkID := decodeBodyMap(t, rec)["id"].(string)
	...
	if decodeBodyMap(t, rec)["deleted"] != true {
...
func TestGetServiceSubscription(t *testing.T) {
	...
	if rec.Code != http.StatusOK {
		t.Fatalf("POST subscription status = %d, want %d: %s", rec.Code, http.StatusOK, rec.Body.String())
	}
	subID := decodeBodyMap(t, rec)["id"].(string)

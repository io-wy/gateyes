...
type ProviderCapabilitiesConfig struct {
	...
	LongContext      *bool `yaml:"longContext"`
...
func loadDotEnv(configPath string) error {
	for _, path := range dotEnvPaths(configPath) {
...
func dotEnvPaths(configPath string) []string {
	configDir := filepath.Dir(configPath)
	paths := []string{filepath.Join(configDir, ".env")}
	if filepath.Base(configDir) == "configs" {
		paths = append(paths, filepath.Join(filepath.Dir(configDir), ".env"))
	}
	return paths
}
